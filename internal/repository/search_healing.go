...
func (r *SearchRepository) searchRules(ctx context.Context, db *gorm.DB, like string, limit int) ([]SearchResultItem, int64, error) {
	var total int64
	db.Model(&model.HealingRule{}).Where("name ILIKE ? OR description ILIKE ?", like, like).Count(&total)
	if total == 0 {
		return nil, 0, nil
	}
...
func (r *SearchRepository) searchFlows(ctx context.Context, db *gorm.DB, like string, limit int) ([]SearchResultItem, int64, error) {
	var total int64
	db.Model(&model.HealingFlow{}).Where("name ILIKE ? OR description ILIKE ?", like, like).Count(&total)
	if total == 0 {
		return nil, 0, nil
	}
...
func (r *SearchRepository) searchInstances(ctx context.Context, db *gorm.DB, like string, limit int) ([]SearchResultItem, int64, error) {
	var total int64
	db.Model(&model.FlowInstance{}).Where("flow_name ILIKE ? OR error_message ILIKE ?", like, like).Count(&total)
	if total == 0 {
		return nil, 0, nil
	}
