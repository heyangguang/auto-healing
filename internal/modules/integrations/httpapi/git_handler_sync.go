...
func (h *GitRepoHandler) SyncRepo(c *gin.Context) {
	...
	if err := h.svc.SyncRepo(c.Request.Context(), id); err != nil {
		respondResourceError(c, "GIT", "同步仓库失败", "仓库不存在", repository.ErrGitRepositoryNotFound, resourceErrorModeInternal, err)
		return
	}
...
func (h *GitRepoHandler) ResetStatus(c *gin.Context) {
	...
	if err := h.svc.ResetStatus(c.Request.Context(), id, targetStatus); err != nil {
		respondResourceError(c, "GIT", "重置仓库状态失败", "仓库不存在", repository.ErrGitRepositoryNotFound, resourceErrorModeInternal, err)
		return
	}
...
func (h *GitRepoHandler) GetCommits(c *gin.Context) {
	...
	if err != nil {
		respondResourceError(c, "GIT", "获取 commit 历史失败", "仓库不存在", repository.ErrGitRepositoryNotFound, resourceErrorModeInternal, err)
		return
	}
...
func (h *GitRepoHandler) GetFiles(c *gin.Context) {
	...
	if path != "" {
		...
		if err != nil {
			respondResourceError(c, "GIT", "获取文件内容失败", "仓库不存在", repository.ErrGitRepositoryNotFound, resourceErrorModeBadRequest, err)
			return
		}
	...
	if err != nil {
		respondResourceError(c, "GIT", "获取文件树失败", "仓库不存在", repository.ErrGitRepositoryNotFound, resourceErrorModeBadRequest, err)
		return
	}
