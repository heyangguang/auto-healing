...
| `close_policy` | object | ❌ | 自动关单策略；启用后会在流程完成时使用指定解决方案模板渲染关闭内容，再调用插件配置的 `close_incident_url` |
...
- `solution_template_id` 绑定的是“解决方案模板”
- 自动关单时，系统会把 `incident / flow / execution / steps` 等运行事实注入模板
- 模板渲染结果会生成结构化回写内容，包含“问题说明 / 解决方案 / 执行步骤 / 验证结果 / 最终结论”
...
**POST** `/api/v1/incidents/:id/restore-trigger`

**权限**: `healing:trigger:execute`

将已忽略且未触发过自愈流程的工单恢复到待触发池。该接口会保留已匹配规则并让工单立即重新出现在待触发工单列表，不会清空匹配状态等待调度器重新扫描。
