...
INSTANCE_STATUS=$(echo "$INSTANCE_DETAIL" | jq -r '.data.status // .status')
CURRENT_NODE=$(echo "$INSTANCE_DETAIL" | jq -r '.data.current_node_id // .current_node_id')
...
if [ -z "$APPROVAL_ID" ] || [ "$APPROVAL_ID" == "" ]; then
  ...
  FINAL_STATUS=$(echo "$FINAL_INSTANCE" | jq -r '.data.status // .status')
  FINAL_NODE=$(echo "$FINAL_INSTANCE" | jq -r '.data.current_node_id // .current_node_id')
