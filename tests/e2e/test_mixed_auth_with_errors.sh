...
# 密码: 192.168.31.103
# 错误（不可达）: 192.168.31.98, 192.168.31.99
...
# 清理
EXISTING=$(curl -s "$API_BASE/secrets-sources" -H "Authorization: Bearer $TOKEN" | jq -r '.data[] | select(.name | startswith("E2E Error Test")) | .id')
for ID in $EXISTING; do curl -s -X DELETE "$API_BASE/secrets-sources/$ID" -H "Authorization: Bearer $TOKEN" > /dev/null; done

EXISTING=$(curl -s "$API_BASE/execution-tasks" -H "Authorization: Bearer $TOKEN" | jq -r '.data[] | select(.name | startswith("E2E Error Test")) | .id')
for ID in $EXISTING; do curl -s -X DELETE "$API_BASE/execution-tasks/$ID" -H "Authorization: Bearer $TOKEN" > /dev/null; done

EXISTING=$(curl -s "$API_BASE/git-repos" -H "Authorization: Bearer $TOKEN" | jq -r '.data[] | select(.name | startswith("E2E Error Test")) | .id')
...
SECRETS_RESULT_SSH=$(curl -s -X POST "$API_BASE/secrets-sources" \
  ...
  -H "Content-Type: application/json" \
  -d "{
    \"name\": \"E2E Error Test SSH Key Source\",
    \"type\": \"webhook\",
    \"auth_type\": \"ssh_key\",
    \"config\": {
...
SECRETS_RESULT_PWD=$(curl -s -X POST "$API_BASE/secrets-sources" \
  ...
  -H "Content-Type: application/json" \
  -d "{
    \"name\": \"E2E Error Test Password Source\",
    \"type\": \"webhook\",
    \"auth_type\": \"password\",
    \"config\": {
      \"url\": \"$MOCK_SECRETS/api/secrets/query\",
...
TASK_RESULT=$(curl -s -X POST "$API_BASE/execution-tasks" \
  ...
  -H "Content-Type: application/json" \
  -d "{
    \"name\": \"E2E Error Test Mixed Auth\",
...
echo ""
echo "--- SSE 实时日志流 ---"
timeout 90s curl -s -N "$API_BASE/execution-runs/$RUN_ID/stream" \
  -H "Authorization: Bearer $TOKEN" \
