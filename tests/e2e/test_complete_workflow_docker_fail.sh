...
if [ -z "$SECRETS_SOURCE_ID" ] || [ "$SECRETS_SOURCE_ID" = "null" ]; then
    echo "  ⚠️ 创建密钥来源失败，尝试使用已有的..."
    SECRETS_SOURCE_ID=$(curl -s -X GET "$API_BASE/api/v1/secrets-sources" \
...
CHANNEL_ID=$(echo "$CHANNEL_RESP" | jq -r '.id // empty')
if [ -z "$CHANNEL_ID" ]; then
    echo "  ⚠️ 创建渠道失败，尝试获取已有渠道..."
    CHANNEL_RESP=$(curl -s "$API_BASE/api/v1/channels" -H "Authorization: Bearer $TOKEN")
    echo "  已有渠道列表:"
    echo "$CHANNEL_RESP" | jq '.items[:3] | .[] | {id, name, type}'
    CHANNEL_ID=$(echo "$CHANNEL_RESP" | jq -r '.items[0].id // empty')
fi
...
TEMPLATE_ID=$(echo "$TEMPLATE_RESP" | jq -r '.id // empty')
if [ -z "$TEMPLATE_ID" ]; then
    echo "  ⚠️ 创建模板失败，尝试获取已有模板..."
    TEMPLATE_LIST=$(curl -s "$API_BASE/api/v1/templates" -H "Authorization: Bearer $TOKEN")
    echo "  已有模板列表:"
    echo "$TEMPLATE_LIST" | jq '.items[:3] | .[] | {id, name, event_type}'
    TEMPLATE_ID=$(echo "$TEMPLATE_LIST" | jq -r '.items[0].id // empty')
fi
...
echo "    url: /root/auto-healing/test-playbook-repo"
echo ""
...
if [ -z "$GIT_REPO_ID" ] || [ "$GIT_REPO_ID" = "null" ]; then
    echo "  ⚠️ 创建失败，使用已有仓库..."
    GIT_REPO_ID=$(curl -s -X GET "$API_BASE/api/v1/git-repos" \
      -H "Authorization: Bearer $TOKEN" | jq -r '.data[0].id // empty')
fi
...
FLOW_RESP=$(curl -s -X POST "$API_BASE/api/v1/healing/flows" \
  ...
  -d '{
    ...
    "nodes": [
      ...
      {"id": "execution_1", "type": "execution", "config": {"git_repo_id": "'$GIT_REPO_ID'", "executor_type": "docker", "hosts_key": "validated_hosts", "secrets_source_id": "'$SECRETS_SOURCE_ID'", "extra_vars": {"service_name": "nginx", "service_action": "check"}}},
...
if [ "$STATUS" = "waiting_approval" ]; then
    ...
    if [ -n "$APPROVAL_TASK_ID" ] && [ "$APPROVAL_TASK_ID" != "null" ]; then
        ...
        # 提取审批任务的关键信息
        APPROVAL_STATUS=$(echo "$APPROVAL_DETAIL" | jq -r '.data.status // .status')
        APPROVAL_NODE_ID=$(echo "$APPROVAL_DETAIL" | jq -r '.data.node_id // .node_id')
        APPROVAL_TIMEOUT=$(echo "$APPROVAL_DETAIL" | jq -r '.data.timeout_at // .timeout_at')
        APPROVAL_CREATED=$(echo "$APPROVAL_DETAIL" | jq -r '.data.created_at // .created_at')
...
if [ -n "$INSTANCE_ID" ] && [ "$INSTANCE_ID" != "null" ]; then
    ...
    echo ""
    echo "  调试信息:"
    echo "    - 查看服务器日志: tail -f /root/auto-healing/server.log | grep -E 'Healing|调度'"
    echo "    - 检查工单是否已被扫描处理"
fi
...
if [ "$NOTIFICATION_COUNT" -gt 0 ]; then
    ...
    echo "  ⚠️ Mock 服务未收到通知 (可能服务未运行或 webhook_url 未配置)"
fi
echo ""
