...
# 清理
EXISTING=$(curl -s "$API_BASE/secrets-sources" -H "Authorization: Bearer $TOKEN" | jq -r '.data[] | select(.name | startswith("E2E SSH Key")) | .id')
for ID in $EXISTING; do curl -s -X DELETE "$API_BASE/secrets-sources/$ID" -H "Authorization: Bearer $TOKEN" > /dev/null; done

EXISTING=$(curl -s "$API_BASE/execution-tasks" -H "Authorization: Bearer $TOKEN" | jq -r '.data[] | select(.name | startswith("E2E SSH Key")) | .id')
for ID in $EXISTING; do curl -s -X DELETE "$API_BASE/execution-tasks/$ID" -H "Authorization: Bearer $TOKEN" > /dev/null; done

EXISTING=$(curl -s "$API_BASE/git-repos" -H "Authorization: Bearer $TOKEN" | jq -r '.data[] | select(.name | startswith("E2E SSH Key")) | .id')
...
echo ""
echo "--- 验证密钥源 ---"
FIRST_HOST=$(echo "$TARGET_HOSTS" | cut -d',' -f1)
...
echo "私钥: ${PRIVATE_KEY:0:50}..."

if [ "$AUTH_TYPE" != "ssh_key" ]; then
  echo "❌ 密钥源验证失败，期望 ssh_key，实际 $AUTH_TYPE"
  exit 1
fi
echo "✅ 密钥源验证成功"
...
if [ "$STATUS" == "success" ]; then
  ...
  echo ""
  echo "  验证内容："
  echo "    1. ✅ 密钥源返回 ssh_key 类型"
  echo "    2. ✅ 后端生成临时密钥文件"
  echo "    3. ✅ Ansible 使用密钥连接成功"
  echo "    4. ✅ 执行成功"
else
  echo "  ❌ SSH 密钥认证测试失败"
  exit 1
